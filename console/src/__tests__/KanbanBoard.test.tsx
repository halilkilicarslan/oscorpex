...
describe('KanbanBoard — yukleme durumu', () => {
  ...
  it('gorevler yuklenirken spinner gosterilmeli', () => {
    ...
    vi.mocked(studioApi.fetchTasks).mockReturnValue(new Promise(() => {}));
  ...
  it('gorevler yuklendikten sonra spinner kaybolmali', async () => {
    vi.mocked(studioApi.fetchTasks).mockResolvedValue(ORNEK_GOREVLER);
...
describe('KanbanBoard — bos durum', () => {
  ...
  it('gorev yoksa bos durum mesaji gosterilmeli', async () => {
    vi.mocked(studioApi.fetchTasks).mockResolvedValue([]);
...
describe('KanbanBoard — sutun yapisi', () => {
  ...
  it('varsayilan sutunlari gostermeli (queued, running, done)', async () => {
    ...
    vi.mocked(studioApi.fetchTasks).mockResolvedValue([ORNEK_GOREVLER[0]]);
...
describe('KanbanBoard — retry islemi', () => {
  ...
  it('retry sonrasi gorevler yeniden yuklenmeli', async () => {
    ...
    await waitFor(() => {
      // fetchTasks en az 2 kez cagrilmali: ilk yukleme + retry sonrasi
      expect(studioApi.fetchTasks).toHaveBeenCalledTimes(2);
    });
  });
});
...
describe('KanbanBoard — periyodik guncelleme', () => {
  ...
  it('5 saniyede bir gorevler yeniden yuklenmeli', async () => {
    vi.mocked(studioApi.fetchTasks).mockResolvedValue(ORNEK_GOREVLER);
    ...
    expect(studioApi.fetchTasks).toHaveBeenCalledTimes(1);
    ...
    expect(studioApi.fetchTasks).toHaveBeenCalledTimes(3);
  });

  it('projectId degisince yeni ID ile fetchTasks cagrilmali', async () => {
    vi.mocked(studioApi.fetchTasks).mockResolvedValue([]);
    ...
    expect(studioApi.fetchTasks).toHaveBeenCalledWith('proj-1');
    ...
    expect(studioApi.fetchTasks).toHaveBeenCalledWith('proj-2');
  });
});
