...
async function exec(
  args: Parameters<NonNullable<typeof dateTimeTool.execute>>[0],
): Promise<DateTimeResult> {
...
describe("dateTimeTool", () => {
  ...
  describe('execute – action "add"', () => {
    ...
    it("throws when 'amount' is not provided", async () => {
      await expect(
        exec({ action: "add", date: "2024-01-01T00:00:00Z", unit: "days" }),
      ).rejects.toThrow(/'amount' is required/);
    });

    it("throws when 'unit' is not provided", async () => {
      await expect(
        exec({ action: "add", date: "2024-01-01T00:00:00Z", amount: 5 }),
