...
export const dateTimeTool = createTool({
	...
	parameters: z.object({
		action: z
			...
			.describe("The date/time action to perform: 'now', 'convert', 'add', or 'difference'."),
		timezone: z
			.string()
			.optional()
			.describe(
		...
		amount: z.number().optional().describe("The amount of time units to add. Required for the 'add' action."),
		unit: z
			.enum(["days", "hours", "minutes"])
			.optional()
			.describe("The time unit for the 'add' action: 'days', 'hours', or 'minutes'."),
	}),
	execute: async ({ action, timezone, date, amount, unit }) => {
		switch (action) {
			...
			case "difference": {
				...
				const result =
					`${totalDays} day(s), ${remainingHours} hour(s), ${remainingMinutes} minute(s)` + ` (${diffMs} ms total)`;
...
function formatDate(date: Date, timezone?: string): string {
	...
	if (timezone) {
		...
		try {
			...
			throw new Error(`Invalid timezone identifier: '${timezone}'. Use an IANA timezone name like 'America/New_York'.`);
		}
	}
