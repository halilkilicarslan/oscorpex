...
const OPENAPI_ENDPOINTS = [
  ...
  '/api/v1/docs', '/api/v1/swagger.json',
];
...
function findRouteFiles(dirPath: string): string[] {
  ...
  for (const file of ['app.js', 'app.ts', 'server.js', 'server.ts', 'index.js', 'index.ts',
...
function parseSourceRoutes(repoPath: string): { routes: DiscoveredRoute[]; basePath: string } | null {
  ...
  for (const mount of allMounts) {
    ...
    const routerFile = files.find(f => {
      const name = (f.split('/').pop()?.replace(/\.(js|ts|mjs)$/, '') || '').toLowerCase();
      return name === routerBase || name === mountBase ||
             routerBase.includes(name) || name.includes(routerBase);
    });
    if (routerFile) {
...
async function probeEndpoints(appUrl: string): Promise<{ routes: DiscoveredRoute[]; basePath: string }> {
  const routes: DiscoveredRoute[] = [];
  const probePaths = [
    '/api/v1/health', '/api/health', '/health', '/healthz',
    '/api/v1', '/api/v2', '/api',
    '/api/v1/users', '/api/v1/items', '/api/v1/products',
    '/api/users', '/api/items', '/api/products',
    ...OPENAPI_ENDPOINTS,
  ];
  ...
  for (const [p, c] of prefixCount) {
    if (c > maxCount) { basePath = p; maxCount = c; }
  }
