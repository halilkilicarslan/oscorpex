...
class ContainerPool {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(`[pool] Initialized with ${this.containers.size} containers (min=${this.config.minIdle}, max=${this.config.maxTotal})`);
  }
  ...
  private async acquire(
    ...
    agentId: string,
    taskId: string,
  ): Promise<PoolContainer | undefined> {
    ...
    try {
      const container = await this.docker.createContainer({
        ...
        Env: [
          `WORKER_PORT=9900`,
          `WORKSPACE=/workspace`,
          `HOST_API=http://host.docker.internal:3141`,
        ],
        HostConfig: {
  ...
  getStatus(): {
    ...
    let ready = 0, busy = 0, unhealthy = 0;
    for (const c of this.containers.values()) {
