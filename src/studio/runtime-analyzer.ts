// ---------------------------------------------------------------------------
// AI Dev Studio — Runtime Analyzer
...
function detectDatabasesFromCompose(repoPath: string): DetectedDatabase[] {
  ...
  if (lower.includes('mongo')) {
    ...
    dbs.push({
      ...
      port: portMatch ? parseInt(portMatch[1]) : 27017,
      envVars: ['MONGO_URI'],
      fromCompose: true,
    });
  }
...
const FRAMEWORK_DEFAULT_PORTS: Partial<Record<FrameworkType, number>> = {
  express: 3000, fastify: 3000, koa: 3000, hono: 3000, nestjs: 3000,
  nextjs: 3000, nuxt: 3000,
  vite: 5173, cra: 3000, angular: 4200,
  django: 8000, flask: 5000, fastapi: 8000,
  'spring-boot': 8080, gin: 8080,
  rails: 3000, 'rust-actix': 8080,
};
...
function detectFramework(dirPath: string, dirName: string): FrameworkDetection | null {
  ...
  if (existsSync(join(dirPath, 'package.json'))) {
    try {
      ...
      if (deps['@nestjs/core']) {
        return {
          name: dirName, framework: 'nestjs', language: 'node',
          startCommand: pkg.scripts?.['start:dev'] ? `${pm} run start:dev` : `${exec} nest start --watch`,
          installCommand: installCmd, readyPattern: 'listening|started|nest application',
          type: 'backend', depsInstalled,
        };
      }
      // Next.js
      if (deps['next']) {
        return {
          name: dirName, framework: 'nextjs', language: 'node',
          startCommand: `${exec} next dev --port \${PORT}`,
          installCommand: installCmd, readyPattern: 'ready|compiled|started',
          type: 'fullstack', depsInstalled,
        };
      }
      // Nuxt
      if (deps['nuxt']) {
        return {
          name: dirName, framework: 'nuxt', language: 'node',
          startCommand: `${exec} nuxi dev --port \${PORT}`,
          installCommand: installCmd, readyPattern: 'ready|listening|local',
          type: 'fullstack', depsInstalled,
        };
      }
      // Vite
      if (deps['vite']) {
        return {
          name: dirName, framework: 'vite', language: 'node',
          startCommand: `${exec} vite --port \${PORT}`,
          installCommand: installCmd, readyPattern: 'ready|local|localhost',
          type: 'frontend', depsInstalled,
        };
      }
      // CRA
      if (deps['react-scripts']) {
        return {
          name: dirName, framework: 'cra', language: 'node',
          startCommand: `${exec} react-scripts start`,
          installCommand: installCmd, readyPattern: 'compiled|webpack|ready',
          type: 'frontend', depsInstalled,
        };
      }
      // Angular
      if (deps['@angular/core']) {
        return {
          name: dirName, framework: 'angular', language: 'node',
          startCommand: `${exec} ng serve --port \${PORT}`,
          installCommand: installCmd, readyPattern: 'compiled|listening|angular',
          type: 'frontend', depsInstalled,
        };
      }
      // Express / Hono / Fastify / Koa
      const backendFramework =
        deps['express'] ? 'express' as const :
        deps['hono'] ? 'hono' as const :
        deps['fastify'] ? 'fastify' as const :
        deps['koa'] ? 'koa' as const : null;

      if (backendFramework) {
        const devScript = pkg.scripts?.dev;
        return {
          name: dirName, framework: backendFramework, language: 'node',
          startCommand: devScript ? `${pm} run dev` : `${exec} tsx src/index.ts`,
          installCommand: installCmd, readyPattern: 'listening|running|started|ready',
          type: 'backend', depsInstalled,
        };
      }
      ...
      if (pkg.scripts?.dev || pkg.scripts?.start) {
        ...
        return {
          name: dirName, framework: 'generic-node', language: 'node',
          startCommand: cmd,
          installCommand: installCmd, readyPattern: 'listening|running|started|ready|compiled',
          type: 'backend', depsInstalled,
        };
      }
    } catch { /* ignore */ }
  ...
  if (hasPython) {
    ...
    const installCmd = useUv ? 'uv sync' : usePoetry ? 'poetry install' : usePipenv ? 'pipenv install' : 'pip install -r requirements.txt';
    const venvExists = existsSync(join(dirPath, '.venv')) || existsSync(join(dirPath, 'venv'));

    // Django
    if (existsSync(join(dirPath, 'manage.py'))) {
      return {
        name: dirName, framework: 'django', language: 'python',
        startCommand: `${prefix} manage.py runserver 0.0.0.0:\${PORT}`,
        installCommand: installCmd, readyPattern: 'starting development server|watching for file changes',
        type: 'backend', depsInstalled: venvExists,
      };
    }
    // FastAPI
    if (existsSync(join(dirPath, 'main.py')) || existsSync(join(dirPath, 'app', 'main.py'))) {
      ...
      return {
        name: dirName, framework: 'fastapi', language: 'python',
        startCommand: `${prefix} uvicorn ${mainFile} --host 0.0.0.0 --port \${PORT} --reload`,
        installCommand: installCmd, readyPattern: 'uvicorn running|started server|application startup',
        type: 'backend', depsInstalled: venvExists,
      };
    }
    // Flask
    if (existsSync(join(dirPath, 'app.py')) || existsSync(join(dirPath, 'wsgi.py'))) {
      return {
        name: dirName, framework: 'flask', language: 'python',
        startCommand: `${prefix} flask run --host 0.0.0.0 --port \${PORT}`,
        installCommand: installCmd, readyPattern: 'running on|debugger is active',
        type: 'backend', depsInstalled: venvExists,
      };
    }
    // Generic Python
    return {
      name: dirName, framework: 'generic-python', language: 'python',
      startCommand: `${prefix} main.py`,
      installCommand: installCmd, readyPattern: 'running|started|listening',
      type: 'backend', depsInstalled: venvExists,
    };
  }
  ...
  if (existsSync(join(dirPath, 'pom.xml'))) {
    ...
    return {
      name: dirName, framework: 'spring-boot', language: 'java',
      ...
      type: 'backend', depsInstalled: existsSync(join(dirPath, 'target')),
    };
  }
  ...
  if (existsSync(join(dirPath, 'build.gradle')) || existsSync(join(dirPath, 'build.gradle.kts'))) {
    ...
    return {
      ...
      readyPattern: 'started|tomcat started|netty started|listening',
      type: 'backend', depsInstalled: existsSync(join(dirPath, 'build')),
    };
  }

  // ---- Go ----
  if (existsSync(join(dirPath, 'go.mod'))) {
    const framework = existsSync(join(dirPath, 'go.sum'))
      && readFileSync(join(dirPath, 'go.sum'), 'utf-8').includes('github.com/gin-gonic/gin')
        ? 'gin' as const : 'go' as const;
    return {
      name: dirName, framework, language: 'go',
      startCommand: 'go run .',
      installCommand: 'go mod download',
      readyPattern: 'listening|started|running|serving',
      type: 'backend', depsInstalled: true, // Go modules auto-download
    };
  }
  ...
  if (existsSync(join(dirPath, 'Gemfile'))) {
    ...
    return {
      name: dirName, framework: isRails ? 'rails' as any : 'unknown', language: 'ruby',
      ...
      type: 'backend', depsInstalled: existsSync(join(dirPath, 'vendor', 'bundle')),
    };
  }
  ...
  if (existsSync(join(dirPath, 'Cargo.toml'))) {
    return {
      name: dirName, framework: 'rust-actix', language: 'rust',
...
export function analyzeProject(repoPath: string): RuntimeRequirements {
  ...
  return {
    ...
    hasDockerCompose: ['docker-compose.yml', 'docker-compose.yaml', 'compose.yml']
 ...
 */
export function generateStudioConfig(
  repoPath: string,
  services: DetectedService[],
