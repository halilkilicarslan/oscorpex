...
import { getProjectSettingsMap, getProjectCostSummary } from '../db.js';
  ...
  describe('budgetGuard', () => {
    ...
    it('should allow when under budget', async () => {
      ...
      mockCostSummary.mockResolvedValue({
        totalCostUsd: 5, totalInputTokens: 1000, totalOutputTokens: 500,
        totalTokens: 1500, taskCount: 3, totalCacheCreationTokens: 0, totalCacheReadTokens: 0,
      });
      const res = await createApp().request('/projects/p1/execute', { method: 'POST' });
      expect(res.status).toBe(200);
    });
    ...
    it('should return 403 when budget exceeded', async () => {
      ...
      mockCostSummary.mockResolvedValue({
        totalCostUsd: 7.5, totalInputTokens: 5000, totalOutputTokens: 2000,
        totalTokens: 7000, taskCount: 10, totalCacheCreationTokens: 0, totalCacheReadTokens: 0,
      });
      const res = await createApp().request('/projects/p1/execute', { method: 'POST' });
