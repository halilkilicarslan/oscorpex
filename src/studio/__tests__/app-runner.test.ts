...
import { resolveConfig, getAppStatus } from '../app-runner.js';
...
describe('app-runner', () => {
  ...
  describe('resolveConfig — .studio.json', () => {
    it('should load .studio.json when present', () => {
      writeFileSync(join(tmpDir, '.studio.json'), JSON.stringify({
        services: [
  ...
  describe('resolveConfig — Docker Compose', () => {
    it('should detect docker-compose.yml with port mappings', () => {
      writeFileSync(join(tmpDir, 'docker-compose.yml'), `
version: "3"
services:
  backend:
    build: ./backend
...
`);
    ...
    it('should detect compose.yml (alternate name)', () => {
      writeFileSync(join(tmpDir, 'compose.yml'), `
services:
  app:
    build: .
    ports:
...
`);
  ...
  describe('resolveConfig — monorepo', () => {
    it('should detect backend + frontend subdirs', () => {
      ...
      writeFileSync(join(tmpDir, 'frontend', 'package.json'), JSON.stringify({
        devDependencies: { vite: '5.0.0' },
      }));
