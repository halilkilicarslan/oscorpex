...
import { ClaudeAdapter, CodexAdapter, AiderAdapter, getAdapter } from '../cli-adapter.js';

vi.mock('../cli-runtime.js', () => ({
  isClaudeCliAvailable: vi.fn().mockResolvedValue(true),
  executeWithCLI: vi.fn().mockResolvedValue({
    text: 'done', filesCreated: [], filesModified: [], logs: [],
    inputTokens: 10, outputTokens: 5, cacheCreationTokens: 0, cacheReadTokens: 0,
    totalCostUsd: 0.001, durationMs: 500, model: 'claude-sonnet-4-6',
  }),
}));
  ...
  describe('ClaudeAdapter', () => {
    ...
    it('should delegate execute to executeWithCLI', async () => {
      const result = await adapter.execute({
        projectId: 'p1', agentId: 'a1', agentName: 'Test',
        repoPath: '/tmp', prompt: 'hello', systemPrompt: 'sys',
        timeoutMs: 5000,
      });
