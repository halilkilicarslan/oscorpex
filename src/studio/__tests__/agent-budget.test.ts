...
const hasDb = await (async () => {
  try { await execute('SELECT 1 FROM token_usage LIMIT 0'); return true; } catch { return false; }
})();
...
describe.skipIf(!hasDb)('Agent Budget (Faz 3.2)', () => {
  ...
  describe('getAgentCostSummary', () => {
    ...
    it('should not cross-contaminate between projects', async () => {
      ...
      const agent1 = await createProjectAgent({
        projectId: p1.id, name: 'Dev1', role: 'developer', avatar: '👩‍💻',
        personality: '', model: 'claude-sonnet-4-6', cliTool: 'claude', skills: [], systemPrompt: '',
      });
      const agent2 = await createProjectAgent({
        projectId: p2.id, name: 'Dev2', role: 'developer', avatar: '👩‍💻',
        personality: '', model: 'claude-sonnet-4-6', cliTool: 'claude', skills: [], systemPrompt: '',
      });

      await recordTokenUsage({
        projectId: p1.id, taskId: 'x-task-1', agentId: agent1.id,
        model: 'claude-sonnet-4-6', provider: 'anthropic',
        inputTokens: 100, outputTokens: 50, totalTokens: 150, costUsd: 0.002,
      });
  ...
  describe('checkProjectBudget — agent-level budget via startTask', () => {
    async function setupWithBudget(agentMaxCostUsd: number) {
      ...
      const agent = await createProjectAgent({
        projectId: project.id, name: 'BudgetCoder', role: 'developer', avatar: '💰',
        personality: '', model: 'claude-sonnet-4-6', cliTool: 'claude', skills: [], systemPrompt: '',
      });
    ...
    it('should block task start when agent budget is exceeded', async () => {
      ...
      await recordTokenUsage({
        projectId: project.id, taskId: 'prev-task', agentId: agent.id,
        model: 'claude-sonnet-4-6', provider: 'anthropic',
        inputTokens: 5000, outputTokens: 2000, totalTokens: 7000, costUsd: 0.05,
      });
    ...
    it('should allow task start when agent budget is not exceeded', async () => {
      ...
      await recordTokenUsage({
        ...
        taskId: 'unrelated', agentId: agent.id,
        model: 'claude-sonnet-4-6', provider: 'anthropic',
        inputTokens: 10, outputTokens: 5, totalTokens: 15, costUsd: 0.0001,
      });
  ...
  describe('budget/status response structure', () => {
    it('should reflect correct project and agent budget info', async () => {
      ...
      const agent = await createProjectAgent({
        projectId: project.id, name: 'StatusAgent', role: 'developer', avatar: '📊',
        personality: '', model: 'claude-sonnet-4-6', cliTool: 'claude', skills: [], systemPrompt: '',
      });
      ...
      await recordTokenUsage({
        projectId: project.id, taskId: 'status-task', agentId: agent.id,
        model: 'claude-sonnet-4-6', provider: 'anthropic',
        inputTokens: 1000, outputTokens: 500, totalTokens: 1500, costUsd: 1.5,
      });
      ...
      expect(agentMaxCost !== null && agentCost.totalCostUsd >= agentMaxCost!).toBe(true); // 1.5 >= 2 is false
    });
    ...
    it('should report agent budget exceeded when cost meets the limit', async () => {
      ...
      const agent = await createProjectAgent({
        projectId: project.id, name: 'ExpensiveAgent', role: 'developer', avatar: '💸',
        personality: '', model: 'claude-sonnet-4-6', cliTool: 'claude', skills: [], systemPrompt: '',
      });

      await recordTokenUsage({
        projectId: project.id, taskId: 'big-task', agentId: agent.id,
        model: 'claude-sonnet-4-6', provider: 'anthropic',
        inputTokens: 10000, outputTokens: 5000, totalTokens: 15000, costUsd: 3.0,
      });
