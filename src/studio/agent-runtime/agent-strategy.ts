...
export const BUILTIN_STRATEGIES: Omit<AgentStrategy, "id">[] = [
	// Backend
	{ agentRole: "backend_dev", name: "test_first", description: "Write tests before implementation. Start with failing tests, then implement to make them pass.", promptAddendum: "IMPORTANT: Write failing tests FIRST, then implement the code to make them pass. Follow TDD strictly.", allowedTaskTypes: ["ai"], isDefault: false },
	{ agentRole: "backend_dev", name: "scaffold_then_refine", description: "Create the basic structure first, then iterate on details. Get the skeleton working before polishing.", promptAddendum: "Start with a minimal working skeleton. Get the basic structure compiling and running first, then add details and error handling.", allowedTaskTypes: ["ai"], isDefault: true },
	{ agentRole: "backend_dev", name: "minimal_patch", description: "Make the smallest possible change to achieve the goal. Minimize blast radius.", promptAddendum: "Make the MINIMUM change needed. Do not refactor surrounding code. Touch as few files as possible.", allowedTaskTypes: ["ai"], isDefault: false },

	// Frontend
	{ agentRole: "frontend_dev", name: "component_first", description: "Build reusable components before composing pages. Focus on component isolation and reusability.", promptAddendum: "Build isolated, reusable components first. Each component should work standalone before being composed into pages.", allowedTaskTypes: ["ai"], isDefault: true },
	{ agentRole: "frontend_dev", name: "page_shell_then_wire", description: "Create the page layout shell first, then wire up data and interactivity.", promptAddendum: "Start with the page layout and static structure. Get the visual layout right, then add data fetching and interactivity.", allowedTaskTypes: ["ai"], isDefault: false },

	// QA / Reviewer
	{ agentRole: "qa_engineer", name: "risk_hotspot_review", description: "Focus review on high-risk code paths: auth, data mutation, external integrations.", promptAddendum: "Prioritize reviewing: authentication/authorization code, database mutations, external API calls, and error handling paths. These are the highest-risk areas.", allowedTaskTypes: ["ai"], isDefault: true },
	{ agentRole: "qa_engineer", name: "test_gap_review", description: "Focus on identifying missing test coverage and untested edge cases.", promptAddendum: "Focus on: Which code paths lack tests? What edge cases are untested? What error scenarios are not covered? Recommend specific tests to add.", allowedTaskTypes: ["ai"], isDefault: false },

	// Tech Lead
	{ agentRole: "tech_lead", name: "spec_contract_first", description: "Define interfaces, types, and contracts before implementation details.", promptAddendum: "Start by defining interfaces, types, and API contracts. Ensure the contract is solid before implementing any logic.", allowedTaskTypes: ["ai"], isDefault: true },
...
export async function selectStrategy(
	...
	const patterns = await getBestStrategies(projectId, agentRole, taskType, 1);
	...
	const builtin = BUILTIN_STRATEGIES.find((s) => s.agentRole === agentRole && s.isDefault);
	if (builtin) {
		return {
			strategy: { ...builtin, id: `builtin-${agentRole}-${builtin.name}` },
