...
export async function startAgent(
	...
	const projectRow = await queryOne<{ repo_path: string }>("SELECT repo_path FROM projects WHERE id = $1", [projectId]);
	...
	const effectiveSystemPrompt = agent.systemPrompt ? composeSystemPrompt(agent.systemPrompt) : undefined;
	const cmdSpec = buildCommand(agent.cliTool, taskPrompt, effectiveSystemPrompt);
	if (!cmdSpec) {
		record.status = "idle";
		return record;
	}
	...
	child.on("exit", (code, signal) => {
		...
		if (stdoutPartial) {
			appendLine(record, stdoutPartial);
			stdoutPartial = "";
		}
		if (stderrPartial) {
			appendLine(record, `[stderr] ${stderrPartial}`);
			stderrPartial = "";
		}
 ...
 */
export function getAgentOutput(projectId: string, agentId: string, since?: number): string[] {
 ...
 */
export function streamAgentOutput(projectId: string, agentId: string): ReadableStream<string> | null {
	const record = processes.get(processKey(projectId, agentId));
 ...
 */
function ensureVirtualProcess(projectId: string, agentId: string, agentName: string): AgentProcessRecord {
	const key = processKey(projectId, agentId);
	const existing = processes.get(key);
	if (existing) {
