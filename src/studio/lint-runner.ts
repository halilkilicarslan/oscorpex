...
const FORMATTABLE_EXTS = new Set([
  '.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs',
  '.css', '.scss', '.json', '.md', '.html', '.yaml', '.yml',
]);
...
function exec(cmd: string, args: string[], cwd: string): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolve) => {
    execFile(cmd, args, { cwd, timeout: 30_000, maxBuffer: 2 * 1024 * 1024 }, (err, stdout, stderr) => {
      const code = err && 'code' in err ? (err as NodeJS.ErrnoException & { code: number }).code as unknown as number : 0;
...
export async function runLintFix(
  ...
  if (formatFiles.length > 0) {
    ...
    try {
      const { stderr, code } = await exec(npx, ['prettier', '--write', '--no-error-on-unmatched-pattern', ...formatFiles], repoPath);
      if (code === 0) {
