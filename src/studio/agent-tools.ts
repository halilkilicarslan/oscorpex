	...
	return {
		listFiles: tool({
			...
			inputSchema: z.object({
				path: z.string().default("").describe("Relative directory path. Empty string for project root."),
			}),
			execute: async ({ path }) => {
				try {
		...
		readFile: tool({
			description: "Read the contents of a file (relative to project root). Returns the file content as a string.",
			inputSchema: z.object({
		...
		runCommand: tool({
			...
			inputSchema: z.object({
				command: z.string().describe('Shell command to execute, e.g. "pnpm install" or "npx tsc --noEmit"'),
			}),
