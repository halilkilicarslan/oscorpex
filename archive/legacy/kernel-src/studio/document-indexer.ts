...
async function indexDocumentContent(
	...
	try {
		...
		await execute(`UPDATE rag_documents SET chunk_count = $1, status = 'ready' WHERE id = $2`, [chunkCount, docId]);
...
async function indexCodebase(options: CodebaseIndexOptions): Promise<CodebaseIndexSummary> {
	...
	try {
		...
		return {
			...
			skippedFiles: 0,
			totalChunks: 0,
			errors: [err?.message ?? "file tree error"],
		};
	}
 ...
 */
async function autoIndexProject(projectId: string, projectPath: string, projectName: string): Promise<string> {
  ...
  `);
	...
	await execute(
     ...
     VALUES ($1, $2, $3, 'code', 'text-embedding-3-small', 512, 50, 'active', 0, 0, NULL, $4, $5)`,
		[kbId, `${projectName} — Codebase`, `Auto-indexed codebase for project: ${projectName} (${projectId})`, now, now],
	);
