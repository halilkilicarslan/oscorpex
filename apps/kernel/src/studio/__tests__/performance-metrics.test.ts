...
describe("buildPerformanceBaseline", () => {
	...
	it("aggregates basic metrics correctly", () => {
		const collector = new ProviderTelemetryCollector();
		const r1 = collector.startExecution({
			runId: "r1",
			taskId: "t1",
		...
		const r2 = collector.startExecution({
			runId: "r2",
			taskId: "t2",
			provider: "claude-code",
			repoPath: "/tmp",
	...
	it("identifies top slowest patterns", () => {
		const collector = new ProviderTelemetryCollector();
		for (let i = 0; i < 3; i++) {
			const r = collector.startExecution({
				runId: `r${i}`,
				taskId: `t${i}`,
				provider: "claude-code",
				repoPath: "/tmp",
			...
			collector.finishExecution(
				r,
				{ provider: "claude-code", output: "", files: [], tokens: {}, metadata: { durationMs: 8000 } } as any,
				new Error("slow"),
			);
		}
		for (let i = 0; i < 3; i++) {
	...
	it("filters by window correctly", () => {
		...
		const old = collector.startExecution({
			runId: "old",
			taskId: "told",
			provider: "claude-code",
			repoPath: "/tmp",
		...
		// Recent record
		const recent = collector.startExecution({
			runId: "new",
			taskId: "tnew",
			provider: "claude-code",
			repoPath: "/tmp",
		...
		collector.finishExecution(recent, {
			provider: "claude-code",
			output: "ok",
			files: [],
			tokens: {},
