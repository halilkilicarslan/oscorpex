...
class ContainerManager {
	...
	async runClaudeCode(
		...
		return this.execCommand(projectId, agentId, ["claude", "--dangerously-skip-permissions", "--message", prompt]);
	}
	...
	async stopAllContainers(projectId: string): Promise<void> {
		...
		await Promise.all(projectRuntimes.map((r) => this.stopContainer(projectId, r.agentId)));
	}
}
