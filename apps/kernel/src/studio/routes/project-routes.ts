...
function readScopeContract(settingsMap: Record<string, Record<string, string>>): ScopeContract | null {
	...
	const status: ScopeStatus =
		statusRaw === "ready_for_review" || statusRaw === "approved" || statusRaw === "superseded"
			? statusRaw
			: "draft";
	return {
...
function toScopeSettingsPayload(scope: Partial<ScopeContract>): Record<string, string> {
	...
	if (scope.requiredCapabilities !== undefined) payload.requiredCapabilities = JSON.stringify(scope.requiredCapabilities);
	if (scope.recommendedTeamRoles !== undefined) payload.recommendedTeamRoles = JSON.stringify(scope.recommendedTeamRoles);
...
projectRoutes.get("/projects/:id/scope", async (c) => {
	const projectId = c.req.param("id");
	const project = await getProject(projectId);
	if (!project) return c.json({ error: "Project not found" }, 404);
	const settingsMap = await getProjectSettingsMap(projectId);
...
projectRoutes.post("/projects/:id/scope/draft", requirePermission("projects:update"), async (c) => {
	const projectId = c.req.param("id");
	const project = await getProject(projectId);
	if (!project) return c.json({ error: "Project not found" }, 404);
...
projectRoutes.post("/projects/:id/scope/approve", requirePermission("projects:update"), async (c) => {
	const projectId = c.req.param("id");
	const project = await getProject(projectId);
	if (!project) return c.json({ error: "Project not found" }, 404);
	const settingsMap = await getProjectSettingsMap(projectId);
...
projectRoutes.post("/projects/:id/team/recommend", requirePermission("projects:update"), async (c) => {
	const projectId = c.req.param("id");
	const project = await getProject(projectId);
	if (!project) return c.json({ error: "Project not found" }, 404);
	...
	const allTemplates = [
		...templates.map((t) => ({ id: t.id, name: t.name, source: "preset" as const, roles: t.roles, description: t.description })),
		...customTemplates.map((t) => ({ id: t.id, name: t.name, source: "custom" as const, roles: t.roles, description: t.description })),
	];
