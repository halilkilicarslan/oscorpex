// ---------------------------------------------------------------------------
// Pipeline Branch Manager — VCS phase hooks
...
const log = createLogger("vcs-phase-hooks");
...
export class PipelineBranchManager {
	...
	async createPhaseBranch(projectId: string, stageIndex: number, stage: PipelineStage): Promise<void> {
		...
		try {
			...
			log.warn(`[vcs-phase-hooks] Branch oluşturulamadı: ${branchName}` + " " + String(err));
		}
	}
	...
	async mergePhaseBranchToMain(projectId: string, stageIndex: number, stage: PipelineStage): Promise<void> {
		...
		try {
			...
			if (result.success) {
				...
				log.warn(
					`[vcs-phase-hooks] Merge conflict tespit edildi: ${branchName} → main` +
						" " +
				...
				await gitManager
					...
					.catch((err) =>
						log.warn("[vcs-phase-hooks] Non-blocking operation failed:", err?.message ?? err),
					);
			}
		} catch (err) {
			log.warn(`[vcs-phase-hooks] Branch merge atlandı: ${branchName}` + " " + String(err));
		}
	}
	...
	async tryCreatePR(projectId: string): Promise<void> {
		...
		log.info(`[vcs-phase-hooks] PR oluşturuldu: ${pr.url}`);
