...
// ---------------------------------------------------------------------------
// Types (mirrored from kernel inspector types — read-only)
// ---------------------------------------------------------------------------
...
export async function fetchTaskInspector(
	projectId: string,
	taskId: string,
): Promise<TaskSessionInspector> {
	const res = await json<{ data: TaskSessionInspector }>(
		`${API}/projects/${projectId}/tasks/${taskId}/inspector`,
	);
	return res.data;
}
